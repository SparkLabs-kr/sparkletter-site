...
## 디자인 (2026-09-17 라이트로 전환)
쿨 뉴트럴 라이트 팔레트. 변수는 `style.css` 최상단 `:root` 한 곳에 있다.

- 배경은 **쿨 near-white(#F7F8FA)**, 텍스트는 **잉크(#14161A)**
- 웜 베이지(본·오트밀)는 요즘 AI가 뽑아내는 화면의 전형이라 피한다
- 액센트(브랜드 핑크 #ED1968)는 **넓게 칠하지 않고** 구독·아카이브 버튼에만 쓴다
- 카드는 흰 면 + 얇은 테두리, 카테고리 색은 **칩에만** 들어간다

> 이전에는 순수 검정 배경에 채도 높은 색으로 카드를 꽉 채웠는데,
> 순수 검정+순백은 헤일레이션으로 눈이 피로하고 액센트가 배경이 돼 강조 기능을 잃는다.
> 읽는 사이트(지난 호 훑어보기)라 라이트가 맞다고 판단했다.
> 밝은 배경이므로 **로고는 원본(logo.png)을 그대로** 쓴다. 흰 글자 가공본은 더 이상 필요 없다.
> 팔레트 후보 비교본은 `mockups/palettes.html`에 남겨두었다.
